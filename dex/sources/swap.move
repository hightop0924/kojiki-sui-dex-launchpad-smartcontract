module defi::kojikiswap {
    ...
    use defi::kojikiswap_library::{quote, sqrt, get_amount_out, get_amount_in, compare, is_overflow_mul, overflow_add};
    use defi::uq64x64;
    // use std::debug;
...
#[test_only]
module defi::kojikiswap_tests {
    use sui::coin::{mint_for_testing as mint, burn_for_testing as burn};
    use sui::coin::{Self, Coin};
    use sui::test_scenario::{Self as test, Scenario, next_tx, ctx};
    use defi::kojikiswap::{Self, LiquidityPools, LPCoin};
    use sui::clock::{Self, Clock};
    // use std::debug;
    ...
    fun test_add_remove_lp_basic() {
        ...
        next_tx(&mut scenario, owner);
        {
            ...
            kojikiswap::init_for_testing(ctx(test));
        ...
        next_tx(&mut scenario, two);
        {
            ...
            kojikiswap::create_pair<TestCoin1, TestCoin2>(&mut lps, ctx(test));
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lp_coins = kojikiswap::add_liquidity<TestCoin1, TestCoin2>(
                &mut lps,
                &clock,
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            let (coin_x, coin_y) = kojikiswap::remove_liquidity<TestCoin1, TestCoin2>(
                &mut lps,
                &clock,
    ...
    fun test_swap_lp_basic() {
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            let (zero, coins_out) = kojikiswap::swap_coins_for_coins<TestCoin1, TestCoin2>(
                &mut lps,
                &clock,
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            let (coins_out, zero) = kojikiswap::swap_coins_for_coins<TestCoin1, TestCoin2>(
                &mut lps,
                &clock,
                coin::zero(ctx(test)),
    ...
    fun test_swap_lp_multiple_exact_x_y_z() {
        ...
        next_tx(&mut scenario, owner);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            kojikiswap::create_pair<TestCoin1, TestCoin2>(&mut lps, ctx(test));
            kojikiswap::create_pair<TestCoin2, TestCoin3>(&mut lps, ctx(test));
            test::return_shared(lps);
        };
        next_tx(&mut scenario, one);
        {
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            let lp_coins = kojikiswap::add_liquidity<TestCoin2, TestCoin3>(
                &mut lps,
                &clock,
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            kojikiswap::swap_exact_coins_for_coins_2_pair_entry<TestCoin1, TestCoin2, TestCoin3>(
                &mut lps,
                &clock,
                mint<TestCoin1>(1000, ctx(test)),
                1000,
    ...
    fun test_swap_lp_multiple_x_y_z_exact() {
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            kojikiswap::swap_coins_for_exact_coins_2_pair_entry<TestCoin1, TestCoin2, TestCoin3>(
                &mut lps,
                &clock,
    ...
    fun test_swap_lp_multiple_exact_z_y_x() {
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            kojikiswap::swap_exact_coins_for_coins_2_pair_entry<TestCoin3, TestCoin2, TestCoin1>(
                &mut lps,
                &clock,
    ...
    fun test_swap_lp_multiple_z_y_x_exact() {
        ...
        next_tx(&mut scenario, one);
        {
            ...
            let lps = test::take_shared<LiquidityPools>(test);
            let clock = test::take_shared<Clock>(test);
            kojikiswap::swap_coins_for_exact_coins_2_pair_entry<TestCoin3, TestCoin2, TestCoin1>(
                &mut lps,
                &clock,
    ...
    #[test]
    #[expected_failure(abort_code = kojikiswap::ERR_PAIR_ALREADY_EXIST)]
